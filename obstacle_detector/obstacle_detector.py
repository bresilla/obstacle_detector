...
class CameraSpinner(Node):
    def __init__(self, args=None):
        ...
        self.img_sub = message_filters.ApproximateTimeSynchronizer(
            [self.color_cam, self.depth_cam], 10, 0.1, allow_headerless=True)
    ...
    def img_callback(self, rgb_msg, depth_msg):
        ...
        if distance == None or distance > 1.0:
            ...
            obs_distance.data = 0.0
            obs_has_size.data = False
            obs_size.data = [0.0, 0.0, 0.0]
            obs_has_position.data = False
            # loc_position.data = [0.0, 0.0, 0.0]
        else:
            obstacle.data = True
            obs_type.data = 1
            obs_detected_before.data = False
